...
import { TunnelRuleParser } from './tunnel-rule.parser';

describe('tunnel-rule.parser', () => {
  describe('#TunnelRuleParser', () => {
    /**
     * Tunnel rule parser parse.
     */
    describe('#TunnelRuleParser::parse', () => {
      it('should return object with three properties', () => {
        const tunnelRuleParser = new TunnelRuleParser();
      ...
      it('should throw error if parts less than three', () => {
        const tunnelRuleParser = new TunnelRuleParser();
        expect(() =>
          tunnelRuleParser.parse('192.168.1.10:22')
        ).to.throw(/tunnel rule is invalid/i);
      });
    });
  });
